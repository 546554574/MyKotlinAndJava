 ...
 */
public class TimePickerViewDay extends BasePickerView implements View.OnClickListener {

    private WheelTimeDay wheelTime; //自定义控件
    private static final String TAG_SUBMIT = "submit";
    private static final String TAG_CANCEL = "cancel";

    public TimePickerViewDay(PickerOptions pickerOptions) {
    ...
    private void initWheelTime(LinearLayout timePickerView) {
        wheelTime = new WheelTimeDay(timePickerView, mPickerOptions.type, mPickerOptions.textGravity, mPickerOptions.textSizeContent);
        if (mPickerOptions.timeSelectChangeListener != null) {
            wheelTime.setSelectChangeCallback(new ISelectTimeCallback() {
                @Override
                public void onTimeSelectChanged() {
                    try {
                        Date date = WheelTimeDay.dateFormat.parse(wheelTime.getTime());
    ...
    public void returnData() {
        if (mPickerOptions.timeSelectListener != null) {
            try {
                ...
                mPickerOptions.timeSelectListener.onTimeSelect(wheelTime.getTime().split(" ")[0]);
            } catch (ParseException e) {
                e.printStackTrace();
            }
        }
    ...
    public void setLunarCalendar(boolean lunar) {
        try {
            int year, month, day, hours, minute, seconds;
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(WheelTimeDay.dateFormat.parse(wheelTime.getTime()));
            year = calendar.get(Calendar.YEAR);
            month = calendar.get(Calendar.MONTH);
            day = calendar.get(Calendar.DAY_OF_MONTH);
            hours = calendar.get(Calendar.HOUR_OF_DAY);
